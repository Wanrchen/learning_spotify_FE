...
@Composable
//这句Kotlin语句定义了一个函数类型的参数onTap，它接受一个Album类型的参数并返回Unit（Kotlin中的Unit类似于Java的void，表示没有任何返回值）。简单来说，这意味着你可以传递一个函数给onTap，这个函数会处理一个Album类型的对象，但不返回任何结果。这常用于设置点击或触摸事件的回调函数。
